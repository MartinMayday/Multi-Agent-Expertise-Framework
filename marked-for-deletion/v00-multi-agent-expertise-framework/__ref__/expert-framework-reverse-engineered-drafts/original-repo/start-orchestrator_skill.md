...
If user request foreground mode, do NOT use `&` suffix and run each command sequentially, starting frontend first in background then backend in foreground:
...
- **Port in use**: The script automatically kill processes using their ports before starting
...
1. Start backend in background:
    ```bash
    cd /path/to/agent-expertise/apps/orchestrator_3_stream && ./start_be.sh &
    ```
...
User request:
```
Start orchestrator with session abc123 and working directory /Users/me/project
```

You would:
1. Start backend with flags:
    ```bash
    cd /path/to/agent-experts/apps/orchestrator_3_stream && ./start_be.sh --cwd /Users/me/project --session abc123 &
    ```
